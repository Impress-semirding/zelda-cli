module.exports = function (points) {
  ...
  const filter_equal_angles = function (p0, Q) {
    ...
    for (let i = 1; i < Q.length; i++) {
      if (Q[i - 1].polar == Q[i].polar) {
        const d1 = distance(p0, Q[i - 1]);
        const d2 = distance(p0, Q[i]);
        if (d2 < d1) {
          Q.splice(i, 1);
        } else {
          Q.splice(i - 1, 1);
  ...
  const cartesian_angle = function (x, y) {
    if (x > 0 && y > 0) { return Math.atan(y / x); } else if (x < 0 && y > 0) { return Math.atan(-x / y) + Math.PI / 2; } else if (x < 0 && y < 0) { return Math.atan(y / x) + Math.PI; } else if (x > 0 && y < 0) { return Math.atan(-x / y) + Math.PI / 2 + Math.PI; } else if (x == 0 && y > 0) { return Math.PI / 2; } else if (x < 0 && y == 0) { return Math.PI; } else if (x == 0 && y < 0) { return Math.PI / 2 + Math.PI; } else return 0;
  };

  const calculate_angle = function (p1, p2) {
    return cartesian_angle(p2[0] - p1[0], p2[1] - p1[1]);
  };

  const calculate_polar_angles = function (p0, Q) {
    for (let i = 0; i < Q.length; i++) {
  ...
  const ccw = function (p1, p2, p3) {
	...
	// Find minimum point
  const Q = points.slice(); // Make copy
  const minIndex = minimum(Q);
  const p0 = Q[minIndex];
