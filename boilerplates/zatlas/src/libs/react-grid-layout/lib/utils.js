 ...
 */
export function compact(layout: Layout, verticalCompact: boolean, cols: number, rows: number): Layout {
  ...
  for (let i = 0, len = sorted.length; i < len; i++) {
    ...
    if (!l.static) {
      l = compactItem(compareWith, l, verticalCompact, cols, rows);
      if (l === -1) {
        return -1;
      }
 ...
 */
export function compactItem(compareWith: Layout, l: LayoutItem, verticalCompact: boolean, cols: number, rows: number): LayoutItem {
  if (verticalCompact) {
 ...
 */
export function moveElement(layout: Layout, l: LayoutItem, x: ?number, y: ?number, cols: number, rows: number, isUserAction: ?boolean): Layout {
  if (l === -1) return -1;
  if (l.static) return layout;

  // Short-circuit if nothing to do.
  if (l.y === y && l.x === x) return layout;
  if (y + l.h > rows || y < 0) return -1;
  ...
  for (let i = 0, len = collisions.length; i < len; i++) {
    ...
    if (collision.static) {
      layout = moveElementAwayFromCollision(layout, collision, l, cols, rows, isUserAction);
    } else {
      layout = moveElementAwayFromCollision(layout, l, collision, cols, rows, isUserAction);
    }
  }
  return layout;
}
...
export function moveElementAwayFromCollision(layout: Layout, collidesWith: LayoutItem,
                                      itemToMove: LayoutItem, cols: number, rows: number,
                                      isUserAction: ?boolean): Layout {
  ...
  if (isUserAction) {
    ...
    if (!getFirstCollision(layout, fakeItem)) {
      return moveElement(layout, itemToMove, undefined, fakeItem.y, cols, rows);
    }
  }
  ...
  //
  if (rows && itemToMove.y + 1 + itemToMove.h >= rows) {
    return -1;
  }
  return moveElement(layout, itemToMove, undefined, itemToMove.y + 1, cols, rows);
}
...
export function synchronizeLayoutWithChildren(initialLayout: Layout, children: ReactChildren,
                                              cols: number, rows: number, verticalCompact: boolean): Layout {
  ...
  layout = compact(layout, verticalCompact, cols, rows);
