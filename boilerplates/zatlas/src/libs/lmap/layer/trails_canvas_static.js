...
var TrailCanvas = require('./trail_canvas_bg');
/**
 * @class TrailsCanvasStatic 大量轨迹的绘制 绘制视窗内所有的轨迹
 */
 ...
 */
function TrailsCanvasStatic(options) {
  options = this.options = Utils.deepMerge(TrailsCanvasStatic.options, options);
  this.initialize(options);
}

TrailsCanvasStatic.options = {
  'lineN': 1000,
  'lifeSpeed': 1,
  'isAutoUpdate': true,
  'type': 'flying',
...
dmap.TrailsCanvasStatic = TrailsCanvasStatic;
dmap.trailsCanvasStatic = function(options) {
  return new TrailsCanvasStatic(options);
};

module.exports = TrailsCanvasStatic;