 ...
 * @Last modified time: 2017-08-29T14:04:51+08:00
* @License: MIT
...
module.exports = (data, types, feedings) => {
  ...
  if (legends.length > 0) {
    if (measures.length === 1) {
      legend = legends[0][1];
      series = legends[0][1].map((v) => {
        const fData = data.filter(e => e[legends[0][0]] === v);
        return {
          name: v,
          stackName: legends[0][0],
          data: uniqCategory.map((value) => {
            const item = find(fData, { [category]: value });
            if (item) {
              return item[measures[0]];
            } else {
              return '-';
            }
          }),
