...
export function compactItem(compareWith: Layout, l: LayoutItem, verticalCompact: boolean, cols: number, rows: number): LayoutItem {
  ...
  while((collides = getFirstCollision(compareWith, l))) {
    console.log(collides, l);
    l.x = collides.x + collides.w;
    if (l.x >= cols) {
      l.x = 0;
      l.y = collides.y + collides.h;
    }
    else if (l.x + l.w > cols) {
      l.w = cols - l.x;
    }
  }
  return l;
...
export function moveElementAwayFromCollision(layout: Layout, collidesWith: LayoutItem,
  ...
  if (isUserAction) {
    ...
    if (!getFirstCollision(layout, fakeItem)) {
      console.log('fake move', fakeItem);
      return moveElement(layout, itemToMove, fakeItem.x);
    }
  }

  // Previously this was optimized to move below the collision directly, but this can cause problems
  // with cascading moves, as an item may actually leapfrog a collision and cause a reversal in order.
  return moveElement(layout, itemToMove, undefined, itemToMove.x + 1);
}
